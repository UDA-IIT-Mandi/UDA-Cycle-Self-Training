...
def main(args: argparse.Namespace):
    ...
    if args.data == 'SVHN_MNIST':
        ...
        unlabeled_transform = TransformFixMatch(grayscale=True)
        
        train_source_dataset = torch_datasets.SVHN(
            root=args.root, split='train', transform=transform, download=True
        )
        train_target_dataset = torch_datasets.MNIST(
            root=args.root, train=True, transform=unlabeled_transform, download=True
        )
        val_dataset = torch_datasets.MNIST(
            root=args.root, train=False, transform=transform, download=True
        )
    ...
    if args.data == 'SVHN_MNIST':
        if hasattr(backbone, 'conv1'):
            ...
            backbone.conv1 = nn.Conv2d(1, old_conv1.out_channels, 
...
#python run_cst.py /path/to/data -d SVHN_MNIST --log cst_svhn_mnist -b 64 --epochs 50